...
Tsub generate_report => << 'END';
    ...
    ARGS {
        ...
        -skip_region_cell_place                 ## to skip the placement for the cells in regions, it would save the load region time
    ...
    print "Log File : $ENV{PWD}/$log_file\n" ;
    ...
    set_chip_top $top;
    my $stype = session_type;

    lprint "# This is $stype session. \n\n";

    lprint "Loading def/region files...\n" ;
    if (defined $ipo_dir && definfed $opt_skip_region_cell_place) {
        load_def_region_files(-ipo_dir => $ipo_dir, -skip_place) ;
    } elsif (defined $ipo_dir) {
        load_def_region_files(-ipo_dir => $ipo_dir) ;
    } elsif (defined $opt_skip_region_cell_place) {
        load_def_region_files(-skip_place) ;
    } else {
...
sub cal_vio_attribute {
    ...
    if ($startpar eq $endpar && $main::p_sig_name{$id} ne "NA" && $main::p_feed_pars_num{$id} > 1 && $is_io == 0) {
        $main::p_start_routeRule{$id}  = GetStartRouteRule($curr_vio) ;
        $main::p_end_routeRule{$id}    = GetEndRouteRule($curr_vio) ;
    ...
    if ($ideal_dist >= $main::share_variables{RETIME_DISTANCE}) {
        ...
        if ($main::p_feed_pars_num{$id} == 2 && $main::p_sig_name{$id} ne "NA" && ($stype eq 'flat' || $stype eq 'ipo')) {
            ...
            if ($start_unit !~ /_retime_partition_/ && $end_unit !~ /_retime_partition_/) {
                if ($main::p_is_mcp{$id}) {
                    ...
                    #print $OUT "Adj nonrt mcp : $main::p_sig_name{$id} attr_of_vio(id => $main::p_AdjParNRT2NRT_mcp{$main::p_sig_name{$id}})\n" if ($debug) ;
                } else {
                    $main::p_AdjParNRT2NRT_nonmcp{$main::p_sig_name{$id}} = $curr_vio ;
                    #print $OUT "Adj nonrt nonmcp : $main::p_sig_name{$id} attr_of_vio(id => $main::p_AdjParNRT2NRT_nonmcp{$main::p_sig_name{$id}})\n" if ($debug) ;
                }
...
sub dump_reports {
    ...
    foreach my $end_clk (sort {$main::p_max_nonrt_nonmcp_clks{$a} <=> $main::p_max_nonrt_nonmcp_clks{$b}} keys %main::p_max_nonrt_nonmcp_clks) {
        ...
        if ($s_type eq 'ipo') {
            printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%10s\t%s\n", "#<Signal Name>", "<Feedthr Partitions>", "<End Clk>", "<Mann Dist>", "<Real Dist>", "<Worst ID>", "is_mcp: <\${is_mcp}>" ) ;  
        } else { 
            printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%10s\t\t%s\n", "#<Signal Name>", "<Feedthr Partitions>", "<End Clk>", "<Mann Dist>", "<Ideal Dist>", "<Worst ID>", "is_mcp: <\${is_mcp}>" ) ;  
        }
        foreach my $sig_name (sort {$main::p_FeedParNRT2NRT_nonmcp{$end_clk}{$b} <=> $main::p_FeedParNRT2NRT_nonmcp{$end_clk}{$a}} keys %{$main::p_FeedParNRT2NRT_nonmcp{$end_clk}}) {
            if ($s_type eq 'ipo') {
                printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%10s\t%s # nonRT  & MCP == 0\n", $sig_name, $main::p_max_nonrt_nonmcp_feed_pars{$sig_name}, $main::p_max_nonrt_nonmcp_end_clk{$sig_name}, $main::p_max_nonrt_nonmcp_man_dist{$sig_name}, $main::p_max_nonrt_nonmcp_real_dist{$sig_name}, $main::p_max_nonrt_nonmcp_worst_id{$sig_name}, "is_mcp: $main::p_nonrt_nonmcp_sig_is_mcp{$sig_name}") ;
            } else {
                printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%10s\t%s # nonRT  & MCP == 0\n", $sig_name, $main::p_max_nonrt_nonmcp_feed_pars{$sig_name}, $main::p_max_nonrt_nonmcp_end_clk{$sig_name}, $main::p_max_nonrt_nonmcp_man_dist{$sig_name}, $main::p_max_nonrt_nonmcp_ideal_dist{$sig_name}, $main::p_max_nonrt_nonmcp_worst_id{$sig_name}, "is_mcp: $main::p_nonrt_nonmcp_sig_is_mcp{$sig_name}") ;
            }
        }
    }
    ...
    foreach my $end_clk (sort {$main::p_max_nonrt_mcp_clks{$a} <=> $main::p_max_nonrt_mcp_clks{$b}} keys %main::p_max_nonrt_mcp_clks) {
        ...
        foreach my $sig_name (sort {$main::p_FeedParNRT2NRT_mcp{$end_clk}{$b} <=> $main::p_FeedParNRT2NRT_mcp{$end_clk}{$a}} keys %{$main::p_FeedParNRT2NRT_mcp{$end_clk}}) {
            if ($s_type eq 'ipo') {
                printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%10s\t%s # nonRT  & MCP == 1\n", $sig_name, $main::p_max_nonrt_mcp_feed_pars{$sig_name}, $main::p_max_nonrt_mcp_end_clk{$sig_name}, $main::p_max_nonrt_mcp_man_dist{$sig_name}, $main::p_max_nonrt_mcp_real_dist{$sig_name}, $main::p_max_nonrt_mcp_worst_id{$sig_name}, "is_mcp: $main::p_nonrt_mcp_sig_is_mcp{$sig_name}") ;
            } else {
                printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%10s\t%s # nonRT  & MCP == 1\n", $sig_name, $main::p_max_nonrt_mcp_feed_pars{$sig_name}, $main::p_max_nonrt_mcp_end_clk{$sig_name}, $main::p_max_nonrt_mcp_man_dist{$sig_name}, $main::p_max_nonrt_mcp_ideal_dist{$sig_name}, $main::p_max_nonrt_mcp_worst_id{$sig_name}, "is_mcp: $main::p_nonrt_mcp_sig_is_mcp{$sig_name}") ;
            }
        }
    }
    ...
    foreach my $end_clk (sort {$main::p_max_rt_nonmcp_clks{$a} <=> $main::p_max_rt_nonmcp_clks{$b}} keys %main::p_max_rt_nonmcp_clks) {
        ...
        if ($s_type eq 'ipo') {
            printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s\n", "#<Signal Name>", "<Feedthr Partitions>", "<End Clk>", "<Mann Dist>", "<Real Dist>", "<Start routeRule>", "<End routeRule>", "<Worst ID>", "is_mcp: <\${is_mcp}>" ) ;  
        } else { 
            printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s\n", "#<Signal Name>", "<Feedthr Partitions>", "<End Clk>", "<Mann Dist>", "<Ideal Dist>", "<Start routeRule>", "<End routeRule>", "<Worst ID>", "is_mcp: <\${is_mcp}>" ) ;  
        }
        ...
        foreach my $sig_name (sort {$main::p_FeedParRT_nonmcp{$end_clk}{$b} <=> $main::p_FeedParRT_nonmcp{$end_clk}{$a}} keys %{$main::p_FeedParRT_nonmcp{$end_clk}}) {
            if ($s_type eq 'ipo') {
                printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s # RT  & MCP == 0\n", $sig_name, $main::p_max_rt_nonmcp_feed_pars{$sig_name}, $main::p_max_rt_nonmcp_end_clk{$sig_name}, $main::p_max_rt_nonmcp_man_dist{$sig_name}, $main::p_max_rt_nonmcp_real_dist{$sig_name}, $main::p_max_rt_nonmcp_s_routeRule{$sig_name}, $main::p_max_rt_nonmcp_e_routeRule{$sig_name}, $main::p_max_rt_nonmcp_worst_id{$sig_name}, "is_mcp: $main::p_rt_nonmcp_sig_is_mcp{$sig_name}") ;
            } else {
    ...
    foreach my $end_clk (sort {$main::p_max_rt_mcp_clks{$a} <=> $main::p_max_rt_mcp_clks{$b}} keys %main::p_max_rt_mcp_clks) {
        ...
        foreach my $sig_name (sort {$main::p_FeedParRT_mcp{$end_clk}{$b} <=> $main::p_FeedParRT_mcp{$end_clk}{$a}} keys %{$main::p_FeedParRT_mcp{$end_clk}}) {
            if ($s_type eq 'ipo') {
                printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s # RT  & MCP == 1\n", $sig_name, $main::p_max_rt_mcp_feed_pars{$sig_name}, $main::p_max_rt_mcp_end_clk{$sig_name}, $main::p_max_rt_mcp_man_dist{$sig_name}, $main::p_max_rt_mcp_real_dist{$sig_name}, $main::p_max_rt_mcp_s_routeRule{$sig_name}, $main::p_max_rt_mcp_e_routeRule{$sig_name}, $main::p_max_rt_mcp_worst_id{$sig_name}, "is_mcp: $main::p_rt_mcp_sig_is_mcp{$sig_name}") ;
            } else {
    ...
    if ($s_type eq 'ipo') {
        printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%s\t%s\t%s\t%s\n", "#<Signal Name>", "<Feedthr Partitions>", "<End Clk>", "<Mann Dist>", "<Real Dist>", "<Start routeRule>", "<End routeRule>", "<Worst ID>", "is_mcp: <\${is_mcp}>" ) ;
    } else {
        printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s\t%s\n", "#<Signal Name>", "<Feedthr Partitions>", "<End Clk>", "<Mann Dist>", "<Ideal Dist>", "<Start routeRule>", "<End routeRule>", "is_mcp: <\${is_mcp}>" ) ;
    }

    foreach my $sig_name (sort keys %main::p_comb_detour_start_routeRule) {
        printf FeedthrParRep ("%-80s\t%-40s\t\t%10s\t%10s\t%10s\t%s\t%s\t%s\n", $sig_name, $main::p_comb_detour_feed_pars{$sig_name}, $main::p_comb_detour_end_clk{$sig_name}, $main::p_comb_detour_man_dist{$sig_name}, $main::p_comb_detour_real_dist{$sig_name}, $main::p_comb_detour_start_routeRule{$sig_name}, $main::p_comb_detour_end_routeRule{$sig_name}, $main::p_comb_detour_worst_id{$sig_name}, "is_mcp: $main::p_comb_detour_is_mcp{$sig_name}") ;
    }
    ...
    if ($s_type eq 'flat' || $s_type eq 'ipo') {
        ...
        foreach my $end_clk (sort {$adj_par_nrt2nrt_nonmcp_clk_period{$a} <=> $adj_par_nrt2nrt_nonmcp_clk_period{$b}} keys %adj_par_nrt2nrt_nonmcp_clk_period) {
            ...
            if ($s_type eq 'ipo') {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s\n", "#<Signal Name>", "<Feedthr Partitions>", "<End Clk>", "<Mann Dist>", "<Real Dist>", "<Source Port Dist>", "<Dest Port Dist>", "<Worst ID>", "is_mcp: <\${is_mcp}>" ) ;
            } else {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s\n", "#<Signal Name>", "<Feedthr Partitions>", "<End Clk>", "<Mann Dist>", "<Ideal Dist>", "<Source Port Dist>", "<Dest Port Dist>", "<Worst ID>", "is_mcp: <\${is_mcp}>" ) ;
            }
            ...
            foreach my $sig_name (sort {$adj_par_nrt2nrt_nonmcp_dist{$end_clk}{$b} <=> $adj_par_nrt2nrt_nonmcp_dist{$end_clk}{$a}} keys %{$adj_par_nrt2nrt_nonmcp_dist{$end_clk}}) {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s # nonRT => nonRT & MCP == 0\n", $sig_name, $main::p_max_feed_pars{$sig_name}, $main::p_max_end_clk{$sig_name}, $main::p_max_man_dist{$sig_name}, $adj_par_nrt2nrt_nonmcp_dist{$end_clk}{$sig_name}, $main::p_max_source_port_dist{$sig_name}, $main::p_max_dest_port_dist{$sig_name}, $adj_par_nrt2nrt_nonmcp_worst_id{$sig_name}, "is_mcp: $main::p_sig_is_mcp{$sig_name}") ;
            }
        }
        ...
        foreach my $end_clk (sort {$adj_par_nrt2nrt_mcp_clk_period{$a} <=> $adj_par_nrt2nrt_mcp_clk_period{$b}} keys %adj_par_nrt2nrt_mcp_clk_period) {
            ...
            foreach my $sig_name (sort {$adj_par_nrt2nrt_mcp_dist{$end_clk}{$b} <=> $adj_par_nrt2nrt_mcp_dist{$end_clk}{$a}} keys %{$adj_par_nrt2nrt_mcp_dist{$end_clk}}) {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s # nonRT => nonRT & MCP == 1\n", $sig_name, $main::p_max_feed_pars{$sig_name}, $main::p_max_end_clk{$sig_name}, $main::p_max_man_dist{$sig_name}, $adj_par_nrt2nrt_mcp_dist{$end_clk}{$sig_name}, $main::p_max_source_port_dist{$sig_name}, $main::p_max_dest_port_dist{$sig_name}, $adj_par_nrt2nrt_mcp_worst_id{$sig_name}, "is_mcp: $main::p_sig_is_mcp{$sig_name}") ;
            }
        }
        ...
        foreach my $end_clk (sort {$adj_par_rt2nrt_nonmcp_clk_period{$a} <=> $adj_par_rt2nrt_nonmcp_clk_period{$b}} keys %adj_par_rt2nrt_nonmcp_clk_period) {
            ...
            if ($s_type eq 'ipo') {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s\t%s\n", "#<Signal Name>", "<Feedthr Partitions>", "<End Clk>", "<Mann Dist>", "<Real Dist>", "<Source Port Dist>", "<Dest Port Dist>", "<Start routeRule>", "<End routeRule>", "<Worst ID>", "is_mcp: <\${is_mcp}>" ) ;
            } else {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s\t%s\n", "#<Signal Name>", "<Feedthr Partitions>", "<End Clk>", "<Mann Dist>", "<Ideal Dist>", "<Source Port Dist>", "<Dest Port Dist>", "<Start routeRule>", "<End routeRule>", "<Worst ID>", "is_mcp: <\${is_mcp}>" ) ;
            }
            ...
            foreach my $sig_name (sort {$adj_par_rt2nrt_nonmcp_dist{$end_clk}{$b} <=> $adj_par_rt2nrt_nonmcp_dist{$end_clk}{$a}} keys %{$adj_par_rt2nrt_nonmcp_dist{$end_clk}}) {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s\t%s # RT => nonRT & MCP == 0\n", $sig_name, $main::p_max_feed_pars{$sig_name}, $main::p_max_end_clk{$sig_name}, $main::p_max_man_dist{$sig_name}, $adj_par_rt2nrt_nonmcp_dist{$end_clk}{$sig_name}, $main::p_max_source_port_dist{$sig_name}, $main::p_max_dest_port_dist{$sig_name}, $main::p_sig_s_routeRule{$sig_name}, $main::p_sig_e_routeRule{$sig_name}, $adj_par_rt2nrt_nonmcp_worst_id{$sig_name}, "is_mcp: $main::p_sig_is_mcp{$sig_name}") ;
            }
        }
        ...
        foreach my $end_clk (sort {$adj_par_rt2nrt_mcp_clk_period{$a} <=> $adj_par_rt2nrt_mcp_clk_period{$b}} keys %adj_par_rt2nrt_mcp_clk_period) {
            ...
            foreach my $sig_name (sort {$adj_par_rt2nrt_mcp_dist{$end_clk}{$b} <=> $adj_par_rt2nrt_mcp_dist{$end_clk}{$a}} keys %{$adj_par_rt2nrt_mcp_dist{$end_clk}}) {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s\t%s # RT => nonRT & MCP == 1\n", $sig_name, $main::p_max_feed_pars{$sig_name}, $main::p_max_end_clk{$sig_name}, $main::p_max_man_dist{$sig_name}, $adj_par_rt2nrt_mcp_dist{$end_clk}{$sig_name}, $main::p_max_source_port_dist{$sig_name}, $main::p_max_dest_port_dist{$sig_name}, $main::p_sig_s_routeRule{$sig_name}, $main::p_sig_e_routeRule{$sig_name}, $adj_par_rt2nrt_mcp_worst_id{$sig_name}, "is_mcp: $main::p_sig_is_mcp{$sig_name}") ;
            }
        }
        ...
        foreach my $end_clk (sort {$adj_par_nrt2rt_nonmcp_clk_period{$a} <=> $adj_par_nrt2rt_nonmcp_clk_period{$b}} keys %adj_par_nrt2rt_nonmcp_clk_period) {
            ...
            foreach my $sig_name (sort {$adj_par_nrt2rt_nonmcp_dist{$end_clk}{$b} <=> $adj_par_nrt2rt_nonmcp_dist{$end_clk}{$a}} keys %{$adj_par_nrt2rt_nonmcp_dist{$end_clk}}) {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s\t%s # nonRT => RT & MCP == 0\n", $sig_name, $main::p_max_feed_pars{$sig_name}, $main::p_max_end_clk{$sig_name}, $main::p_max_man_dist{$sig_name}, $adj_par_nrt2rt_nonmcp_dist{$end_clk}{$sig_name}, $main::p_max_source_port_dist{$sig_name}, $main::p_max_dest_port_dist{$sig_name}, $main::p_sig_s_routeRule{$sig_name}, $main::p_sig_e_routeRule{$sig_name}, $adj_par_nrt2rt_nonmcp_worst_id{$sig_name}, "is_mcp: $main::p_sig_is_mcp{$sig_name}") ;
            }
        }
        ...
        foreach my $end_clk (sort {$adj_par_nrt2rt_mcp_clk_period{$a} <=> $adj_par_nrt2rt_mcp_clk_period{$b}} keys %adj_par_nrt2rt_mcp_clk_period) {
            ...
            foreach my $sig_name (sort {$adj_par_nrt2rt_mcp_dist{$end_clk}{$b} <=> $adj_par_nrt2rt_mcp_dist{$end_clk}{$a}} keys %{$adj_par_nrt2rt_mcp_dist{$end_clk}}) {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s\t%s # nonRT => RT & MCP == 1\n", $sig_name, $main::p_max_feed_pars{$sig_name}, $main::p_max_end_clk{$sig_name}, $main::p_max_man_dist{$sig_name}, $adj_par_nrt2rt_mcp_dist{$end_clk}{$sig_name}, $main::p_max_source_port_dist{$sig_name}, $main::p_max_dest_port_dist{$sig_name}, $main::p_sig_s_routeRule{$sig_name}, $main::p_sig_e_routeRule{$sig_name}, $adj_par_nrt2rt_mcp_worst_id{$sig_name}, "is_mcp: $main::p_sig_is_mcp{$sig_name}") ;
            }
        }
        ...
        foreach my $end_clk (sort {$adj_par_rt2rt_nonmcp_clk_period{$a} <=> $adj_par_rt2rt_nonmcp_clk_period{$b}} keys %adj_par_rt2rt_nonmcp_clk_period) {
            ...
            foreach my $sig_name (sort {$adj_par_rt2rt_nonmcp_dist{$end_clk}{$b} <=> $adj_par_rt2rt_nonmcp_dist{$end_clk}{$a}} keys %{$adj_par_rt2rt_nonmcp_dist{$end_clk}}) {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s\t%s # RT => RT & MCP == 0\n", $sig_name, $main::p_max_feed_pars{$sig_name}, $main::p_max_end_clk{$sig_name}, $main::p_max_man_dist{$sig_name},$adj_par_rt2rt_nonmcp_dist{$end_clk}{$sig_name}, $main::p_max_source_port_dist{$sig_name}, $main::p_max_dest_port_dist{$sig_name}, $main::p_sig_s_routeRule{$sig_name}, $main::p_sig_e_routeRule{$sig_name}, $adj_par_rt2rt_nonmcp_worst_id{$sig_name}, "is_mcp: $main::p_sig_is_mcp{$sig_name}") ;
            }
        }
        ...
        foreach my $end_clk (sort {$adj_par_rt2rt_mcp_clk_period{$a} <=> $adj_par_rt2rt_mcp_clk_period{$b}} keys %adj_par_rt2rt_mcp_clk_period) {
            ...
            foreach my $sig_name (sort {$adj_par_rt2rt_mcp_dist{$end_clk}{$b} <=> $adj_par_rt2rt_mcp_dist{$end_clk}{$a}} keys %{$adj_par_rt2rt_mcp_dist{$end_clk}}) {
                printf AdjacentParRep ("%-80s\t%-40s\t\t%10s%10s\t%10s\t%10s\t%10s\t%s\t%s\t%s\t%s # RT => RT & MCP == 1\n", $sig_name, $main::p_max_feed_pars{$sig_name}, $main::p_max_end_clk{$sig_name}, $main::p_max_man_dist{$sig_name}, $adj_par_rt2rt_mcp_dist{$end_clk}{$sig_name}, $main::p_max_source_port_dist{$sig_name}, $main::p_max_dest_port_dist{$sig_name}, $main::p_sig_s_routeRule{$sig_name}, $main::p_sig_e_routeRule{$sig_name}, $adj_par_rt2rt_mcp_worst_id{$sig_name}, "is_mcp: $main::p_sig_is_mcp{$sig_name}") ;
            }
        }
