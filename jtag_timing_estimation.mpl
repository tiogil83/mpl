...
if ($#def_files == -1) {
    ...
    if ($type eq 'noscan') {
        ...
        load "/home/scratch.gp104_master/gp104/gp104/timing/gp104/lib/NV_BLKBOX_BUFFER_tsmc16ff_t9_svt_std_ssg_0c_0p6v.lib" ;
        set_timing_corner ssg_0c_0p6v ;
        # special for lr10, no 0p6v corner libs available.
        if ($proj eq 'lr10') {
            load "/home/scratch.gp104_master/gp104/gp104/timing/gp104/lib/NV_BLKBOX_BUFFER_tsmc16ff_t9_svt_std_ssg_105c_0p8v.lib" ;
            set_timing_corner ssg_105c_0p8v ;
        }
    }
...
## to enable the timing arc for BLKBOX in noscan NL
#if ($type eq 'noscan') {
#    set_timing_corner ssg_0c_0p6v ;
#}
...
$jtag_clock_source{$top_level} = $jtag_clock_source_top[0] ;
    ...
    foreach my $start_clk_pin (sort keys %all_wsi_paths) {
        ...
        foreach my $end_clk_pin (sort keys %$end_clk_pins) {
            ...
            if (!(defined $opt_viol_only) || $path_slack < 0) {
                ...
                if (defined $opt_plot) {
                    plot_path ($start_clk_pin, $end_clk_pin, "wsi_ordering_${start_par}_to_${end_par}", "red") ;
                }
            }
        }
    }
    ...
    foreach my $start_clk_pin (sort keys %all_wsc_paths) {
        ...
        foreach my $end_clk_pin (sort keys %$end_clk_pins) {
            ...
            if (!(defined $opt_viol_only) || $path_slack < 0) {
                ...
                if (defined $opt_plot) {
                    plot_wsc_path ((get_path_ordering_pin (get_all_pins_of_path ($start_q_pin, $end_d_pin))), "wsc_ordering_${start_par}_to_${end_par}", "red") ;
                }
            }
        }
    }
    ...
    foreach my $start_clk_pin (sort keys %all_retime_paths) {
        ...
        foreach my $end_clk_pin (sort keys %$end_clk_pins) {
            ...
            if (!(defined $opt_viol_only) || $path_slack < 0) {
                ...
                if (defined $opt_plot) {
                    plot_wsc_path ((get_path_ordering_pin (get_all_pins_of_path ($start_q_pin, $end_d_pin))), "retime_ordering_${start_par}_to_${end_par}", "red") ;
                }
            }
        }
    }
